...
export default async function DynamicRoute({params}) {
    ...
    .set({
      ...
      poster: formData.get("user"),
      post: formData.get("message"),
      updated: true
      })
      .where(eq(blogPosts.id, params.data));
      redirect(`/pages/blogs/read/${params.data}`, RedirectType.replace);
  }
  
    async function DeleteBlog() {
      "use server";
      await db.delete(blogComments).where(eq(blogComments.blogId, params.data));
      await db.delete(blogPosts).where(eq(blogPosts.id, params.data));
...
// //   console.log('Blog Post deleted!')
  ...
  return (
      ...
      <ul>
        {data.map((info) => (
          <div key={info.id} className="py-8 lg:py-16 px-4 mx-auto max-w-screen-md">
            <h2 className="mb-4 text-4xl tracking-tight font-extrabold text-center text-gray-900 dark:text-white">Edit Your Blog Post</h2>
            <p className="mb-8 lg:mb-16 font-light text-center text-gray-500 dark:text-gray-400 sm:text-xl">Change the deatils about your Blog.</p>
            <form action={UpdateBlog} className="space-y-8">
                <div>
                  <label htmlFor="user" className="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">Username</label>                
                  <input defaultValue={`${info.poster}`} type="text" id="user" name="user" className="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500 dark:shadow-sm-light" placeholder="username" required></input>
                  <br/>
                  ...
                  <button value="40" type="button" radioGroup="color" className="text-white bg-purple-700 hover:bg-purple-800 focus:outline-none focus:ring-4 focus:ring-purple-300 font-medium rounded-full text-sm px-5 py-2.5 text-center mb-2 dark:bg-purple-600 dark:hover:bg-purple-700 dark:focus:ring-purple-900">Purple</button>
                    ...
                    <input defaultValue={`${info.title}`} type="text" id="title" name="title" className="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500 dark:shadow-sm-light" placeholder="title" required></input>
                    <br/>
                    <div className="sm:col-span-2">
                        <label htmlFor="message" className="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">Message</label>
                        <textarea defaultValue={`${info.post}`} id="message" name="message" rows={6} className="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg shadow-sm border border-gray-300 focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="blog"></textarea>
                    </div>
                </div>
            ...
            <button className="focus:outline-none text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 mb-2 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900" onClick={DeleteBlog}>Delete Blog</button>
        </div>
        ))}
      </ul>
    </div>
